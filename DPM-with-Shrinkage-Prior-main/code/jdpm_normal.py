...
#        likelihood = likelihood_y * likelihood_x        
    ...
    eta = np.random.beta(b+1,n_clus)
    u_tmp = np.random.uniform(0,1)
    O = (b_alpha+K-1) / ((b_beta-np.log(eta))*n_clus)
...
# # =============================================================================
# x_new = np.ones(p) * 2
# y_grid = np.linspace(30, 50, 100)
# y_pdf_true = norm.pdf(y_grid, loc = mu_true[0] + np.dot(x_new, beta_true[0,:]), scale = np.sqrt(sigma2_true[0]))

# # approximate integral part in the first term of f[Y|x,\theta_1:n]
# def cal_integral_pdf(x_new,y_grid,i,num_MCMC,burn_in):
#     integral_sum = 0
#     for s in range(num_MCMC - burn_in):
#         mu_pred = reg_mu_bnp[burn_in+s]
#         beta_pred = reg_beta_bnp[:,burn_in+s]
#         sigma2_pred = reg_sigma2_bnp[burn_in+s]
#         m_pred = reg_m_bnp[:,burn_in+s]
#         tau_pred = reg_tau_bnp[:,burn_in+s]
        
#         tmp_x_prod = 1
#         for l in range(p):
#             tmp_x_prod = tmp_x_prod * norm.pdf( x_new[l], loc = m_pred[l], scale = np.sqrt(tau_pred[l]) )
#         integral_sum = integral_sum + norm.pdf( y_grid[i], loc = mu_pred + np.dot(x_new,beta_pred), scale = np.sqrt(sigma2_pred) ) * tmp_x_prod
#     integral_pred = integral_sum / (num_MCMC -  burn_in)
#     return integral_pred
    
# # direct calculate integral part in the first term of b
# def cal_integral_weight_pdf(x_new,i,num_MCMC,burn_in):
#     integral_weight = 1
#     for l in range(p):
#         integral_weight = integral_weight * scipy.stats.t.pdf(x_new[l], df = nu_0[l], loc = m_0[l], scale = np.sqrt(s2_0[l]**2 + (s2_0[l]**2)/n_0[l]))
#     return integral_weight

# # store pdf predictions for each iteration in mcmc

# y_pdf = np.zeros( (y_grid.shape[0],num_MCMC-burn_in) )

# for i in range(y_grid.shape[0]):
#     print(i)
#     integral_pdf = cal_integral_pdf(x_new,y_grid,i,num_MCMC,burn_in)
#     integral_weight = cal_integral_weight_pdf(x_new,i,num_MCMC,burn_in)
#     # approximate with MCMC
#     y_pdf_tmp = []
#     for s in range(num_MCMC - burn_in):
#         lib = Counter(di_bnp[:,burn_in+s]) # count size and index of clusters
#         b_second = 0  # second term in b
#         pdf_second = 0  # second term in pdf
#         for key in lib.keys():
#             key = int(key)
#             b_tmp = 1
#             second_tmp = 1
#             for l in range(p):
#                 b_tmp = b_tmp * norm.pdf( x_new[l], loc = m_cand[burn_in+s,l,key], scale = np.sqrt(tau_cand[burn_in+s,l,key]))
#             second_tmp = b_tmp * norm.pdf(y_grid[i], loc = mu_cand[burn_in+s,:,key] + np.dot(x_new,beta_cand[burn_in+s,:,key]), scale = np.sqrt(sigma2_bnp[burn_in+s]) )
#             b_second = b_second + b_tmp * lib[key]
#             pdf_second = pdf_second + second_tmp * lib[key]
#         b_total = b_bnp[burn_in+s] * integral_weight + b_second
#         y_pdf_tmp.append( 1 / b_total * ( b_bnp[burn_in+s] * integral_pdf + pdf_second ) )
# #    y_pdf_tmp = np.array(y_pdf_tmp)
#     y_pdf[i,:] = y_pdf_tmp


# y_pdf_mean = np.mean(y_pdf, axis = 1)
# y_pdf_low, y_pdf_high = np.percentile(y_pdf, [2.5, 97.5], axis=1)


# ###### plot y_grid against y's true pdf, estimated pdf, and 95% credible interval
# fig, ax = plt.subplots(figsize=(8, 6))
# ax.scatter(y_grid, y_pdf_mean, color='red', alpha=1, marker = '.', label = 'estimate');
# ax.plot(y_grid,y_pdf_true, color='grey', linestyle = 'solid', lw=2, alpha=1, label = 'true');
# #ax.errorbar(x_test_tmp,y_pred_mean_tmp, yerr = y_error, fmt='.b', ecolor = 'blue')
# ax.plot(y_grid,y_pdf_low, color='grey', linestyle = 'dashed', lw=1, alpha=0.5, label = '95% credible interval');
# ax.plot(y_grid,y_pdf_high, color='grey', linestyle = 'dashed', lw=1, alpha=0.5);

# ax.set_xlabel('y');
# ax.set_ylabel('f(y|x)');
# ax.legend();
# plt.savefig(dir_current + "pdf.png")

# ### save y_pdf_true and y_pdf
# obj_to_save = (y_pdf_true, y_pdf)
# with open(dir_current + "y_pdf_true_y_pdf_n%i_p%i_cluster%i.pickle" %(n,p,n_clus),"wb") as f:
#    pickle.dump(obj_to_save, f)
